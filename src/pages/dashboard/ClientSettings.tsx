          ...
          <div className="mb-6 flex items-center gap-6">
            <AvatarUpload 
              ...
              onUpload={async (url) => {
                ...
                if (user) {
                  await supabase.from('client_profiles').update({ avatar_url: url }).eq('id', user.id);
                  window.dispatchEvent(new Event('profileUpdated'));
                }
              }} 
              size={100}
        ...
        <div className="bg-white p-6 md:p-8 rounded-xl border border-border shadow-sm">
          ...
          <div className="mb-6 flex items-center gap-6">
            <AvatarUpload 
              ...
              onUpload={async (url) => {
                setProfile({...profile, company_logo: url});
                if (user) {
                  await supabase.from('client_profiles').update({ company_logo: url }).eq('id', user.id);
                  window.dispatchEvent(new Event('profileUpdated'));
                }
              }} 
              size={80}
